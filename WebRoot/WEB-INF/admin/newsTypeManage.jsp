...
<script type="text/javascript">
	...
	function searchnewsType(){
		$('#dg').datagrid('load',{
			"s_newsType.name":$("#s_newsTypeName").val()
		});
	}
	
	function opennewsTypeAddDialog(){
		$("#dlg").dialog("open").dialog("setTitle","添加商品大类信息");
		url="newsType_save.action";
	}
	...
	function opennewsTypeModifyDialog(){
		...
		$("#name").val(row.typeName);
		url="newsType_save.action?newsType.id="+row.id;
	}
	...
	function deletenewsType(){
		...
		$.messager.confirm("系统提示","您确认要删掉这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("newsType_delete.action",{ids:ids},function(result){
					if(result.success){
						if(result.exist){
...
<table id="dg" title="商品大类管理" class="easyui-datagrid" fitColumns="true" 
    pagination="true" rownumbers="true" url="newsType_list.action" fit="true" toolbar="#tb">
    <thead>
    	<tr>
...
<div id="tb">
	...
	<div>
		&nbsp;新闻类别名称：&nbsp;<input type="text" name="newsType.typeName"  id="s_newsTypeName"  size="20" onkeydown="if(event.keyCode==13) searchnewsType()"/>
		<a href="javascript:searchnewsType()" class="easyui-linkbutton" iconCls="icon-search" plain="true">搜索</a>
	</div>
</div>
...
<div id="dlg" class="easyui-dialog" style="width: 570px;height: 300px;padding: 10px 20px"
  ...
  <form id="fm" method="post">
  	<table cellspacing="8px;">
  		<tr>
  			<td>类别名称：</td>
  			<td colspan="4"><input type="text" id="name" name="newsType.typeName" class="easyui-validatebox" required="true"/></td>
  		</tr>
  		<!-- <tr>
  			<td valign="top">备注：</td>
  			<td colspan="4">
  				<textarea rows="7" cols="50" name="newsType.remarks" id="remarks"></textarea>
  			</td>
  		</tr> -->
  	</table>
  </form>
