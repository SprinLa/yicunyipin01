...
.policyList{margin-top:20px;}
.policyList ul.policy li { margin-left:180px;width:619px;line-height:42px; font-size:13px; overflow:hidden;border-bottom:1px solid #ffefd2; }
.policyList ul.policy li span {float:right }
...
<body>
<div class="main"style="border:1px solid #bfbfbf;width:1200px">
	<div class="policyList">
			<!-- <h2>全部商品</h2> -->
			<ul class="policy clearfix">
				<c:forEach items="${policyList}" var="p">
					<li>
								<a href="policy_showPolicy.action?policyId=${p.id }" target="_blank">${p.title}</a><span><fmt:formatDate value="${p.addTime }" type="date" pattern="yyyy-MM-dd"/></span>
					</li>
				</c:forEach>
			</ul>
			<div class="clear"></div>
